...
$jml_jam = $end - $start;
$sql = mysql_query("SELECT * FROM reservasi, lapangan WHERE reservasi.id_lapangan = lapangan.id_lapangan");
$r2 = mysql_fetch_array($sql);
$total = $jml_jam * $r2['harga'];
  
$tgl_s = strtotime($tgl);
$j_start_s = strtotime($start);
$j_end_s = strtotime($end);
...
//$pengurangan = strtotime(("$kurangi_enam:00:00"));
  //Cek apakah lapangan dengan tanggal dan jam sekian tersedia atau tidak
  if ($lap = $r2['id_lapangan'] && $tgl_r=$tgl_s && $jam_m_r=$j_start_s && $jam_s_r=$j_end_s) {
  ...
  } elseif ($lap = $r2['id_lapangan'] && $tgl_r=$tgl_s && $j_start_s < $jam_s_r) {
    echo "<script>alert('Maaf, Lapangan dengan kode $lap tidak tersedia (Not Available)!. Silahkan lakukan reservasi dengan waktu yang berbeda.');</script>";
    echo "<script>history.go(-1);</script>";
  //Cek apakah jam selesai tidak lebih kecil dari jam mulai
    ...
    echo "<script>history.go(-1);</script>";
  //Reservasi dilakukan minimal 6 jam sebelum bermain
  //} elseif ($tgl_s = $t_now AND $j_start_s > $pengurangan) {
    //echo "<script>alert('Maaf, Reservasi dilakukan minimal 6 jam sebelum bermain.');</script>";
    //echo "<script>history.go(-1);</script>";
  //Cek jika semua tidak sama dengan yang di database
  } else {    
    if ($tgl_s == $t_now AND $j_start_s < $j_now) { //bila tanggal tepat hari ini maka jam harus lebih besar dari jam sekarang      
